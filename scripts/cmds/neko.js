...
module.exports = {
	config: {
		...
		countDown: 5,
		role: 0,
		shortDescription: 'neko',
		longDescription: 'Chat with neko',
		category: 'funny',
		guide: {
			body: '{pn} {{[on | off]}}: enable/disable neko'
				+ '\n'
				+ '\n   {pn} {{<word>}}: quick chat with Neko'
				+ '\n   Example: {pn} {{hi}}'
		}
	},
	...
	onStart: async function ({ args, threadsData, message, event }) {
		if (args[0] == 'on' || args[0] == 'off') {
			...
			return message.reply(`Already ${args[0] == "on" ? "on" : "off"} Neko in your group`);
		}
		else if (args[0]) {
			...
			try {
				...
				return message.reply("Neko is busy, please try again later");
			}
		}
	},
